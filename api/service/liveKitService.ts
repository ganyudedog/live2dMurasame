...
const readResponseByType = async (response: Response, responseType: NonNullable<HttpRequestOptions['responseType']>) => {
...
export const requestViaFetch = async <T>({ method, url, body, responseType = 'json', signal }: HttpRequestOptions): Promise<T> => {
    try {
        ...
        if (!response.ok) {
            const maybeErr = rawResponse as { error?: { code?: string; message?: string; }; } | null;
        ...
        throw new LiveKitApiError(String(err instanceof Error ? err.message : err), 0);
    }
};

export const toSessionCreateServer = (payload?: LiveKitSessionCreatePayload): LiveKitSessionCreatePayloadServer => {
    return {
...
export const fromSessionCreateServer = (raw: LiveKitSessionCreateResponseServer): LiveKitSessionCreateResponse => {
    return {
...
export const toModelSwitchServer = (request: LiveKitModelSwitchRequest): LiveKitModelSwitchRequestServer => {
    return {
...
export const fromModelSwitchServer = (raw: LiveKitModelSwitchResponseServer): LiveKitModelSwitchResponse => {
    return {
        ok: raw.ok,
...
export const toTtsSpeakServer = (request: LiveKitTtsSpeakRequest): LiveKitTtsSpeakRequestServer => {
    return {
        session_id: request.sessionId,
        request_id: request.requestId,
        ts: request.ts,
...
export const toTtsCancelServer = (request: LiveKitTtsCancelRequest): LiveKitTtsCancelRequestServer => {
    return {
        session_id: request.sessionId,
        request_id: request.requestId,
        ts: request.ts,
...
export const getJson = async <T>(baseUrl: string, path: string, signal?: AbortSignal): Promise<T> => {
    const url = `${normalizeBaseUrl(baseUrl)}${path}`;
    return requestViaFetch<T>({
        method: 'GET',
        url,
