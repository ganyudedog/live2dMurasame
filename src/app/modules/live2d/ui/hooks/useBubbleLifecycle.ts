...
export const useBubbleLifecycle = ({
  ...
  useLayoutEffect(() => {
    ...
    if (!motionText) {
      ...
      clearBubbleTimer();
      return () => {
        if (resetRafId !== null) window.cancelAnimationFrame(resetRafId);
      };
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      layoutRafId = window.requestAnimationFrame(scheduleLayoutRefresh);
    } else {
      scheduleReadyReset();
    ...
    return () => {
      ...
      if (layoutRafId !== null) window.cancelAnimationFrame(layoutRafId);
