...
- 新增全局配置文件：`liv2denv.json`。
	...
	- 不再包含 `GLOBAL`（见下方独立 settings 文件）。

- 新增全局模型设置文件：`globalModelConfig.json`。
	- 内容：`scale/ignoreMouse/autoLaunch/showDragHandleOnHover/forcedFollow/debugModeEnabled`。
...
- 写入流程：
	- 控制面板修改模型参数时，只更新模型专属文件；涉及应用级配置（模型列表/当前模型等）写回 `liv2denv.json`；涉及全局模型设置写回 `globalModelConfig.json`。
	- 使用防抖/批量写入，避免频繁 IO。
